...
class AdminController {
    ...
    async login(req, res) {
        try {
            ...
            // Optionnel : stocker le refresh token dans un cookie sécurisé
            ...
            return res.status(200).json({
                ...
                refreshToken: data.refreshToken, // On le renvoie aussi si le client ne veut pas de cookie
                user: data.user
            });
    ...
    async logout(req, res) {
        try {
            const accessToken = req.token; // Extrait par le middleware verifyToken
            const refreshToken = req.body.refreshToken || req.cookies.refreshToken;
    ...
    async getProfile(req, res) {
        try {
            // req.user est injecté par le middleware d'authentification (prochaine étape)
    ...
    /**
     * @route   PATCH /api/admin/update-profile
